...
local Config={
 WindowSize=UDim2.new(0,560,0,360),BgColor=Color3.fromRGB(15,17,22),CardBg=Color3.fromRGB(22,25,33),
...
-- Türkçe: Log için sadece kopyalanabilir debug kodu üretir; çağrıyı çalıştırmaz.
local function BuildCode(remote,kind,args)
 local lines={"-- Remote: "..(pcall(function()return remote:GetFullName()end) and remote:GetFullName() or "Unknown"),"-- Çağrı: "..kind,"-- Zaman: "..os.date("%H:%M:%S"),"","local args = {"}
 for i,v in ipairs(args or {}) do table.insert(lines,string.format("    [%d] = %s,",i,Serialize(v,1,{}))) end
 table.insert(lines,"}");table.insert(lines,"");table.insert(lines,"-- Safe Debug: replay/run disabled.");return table.concat(lines,"\n")
end
...
local function ExecuteScript()
 ...
 local Status=Instance.new("TextLabel");Status.Size=UDim2.new(0,180,1,0);Status.Position=UDim2.new(0,195,0,0);Status.BackgroundTransparency=1;Status.Text="● DEBUG";Status.TextColor3=Config.EventColor;Status.Font=Enum.Font.Gotham;Status.TextSize=9;Status.TextXAlignment=Enum.TextXAlignment.Left;Status.Parent=Bar
 local Close=Instance.new("TextButton");Close.Size=UDim2.new(0,30,0,30);Close.Position=UDim2.new(1,-32,0,2);Close.BackgroundTransparency=1;Close.Text="×";Close.TextColor3=Config.CloseRed;Close.Font=Enum.Font.GothamBold;Close.TextSize=20;Close.Parent=Bar
 local Min=Instance.new("TextButton");Min.Size=UDim2.new(0,30,0,30);Min.Position=UDim2.new(1,-62,0,2);Min.BackgroundTransparency=1;Min.Text="-";Min.TextColor3=Config.TextSecondary;Min.Font=Enum.Font.GothamBold;Min.TextSize=18;Min.Parent=Bar
 ...
 local function Tab(name,order)
  local b=Instance.new("TextButton");b.Size=UDim2.new(0,110,1,0);b.BackgroundColor3=Config.CardBg;b.Text=name;b.TextColor3=Config.TextSecondary;b.Font=Enum.Font.GothamMedium;b.TextSize=10;b.LayoutOrder=order;b.AutoButtonColor=false;b.Parent=TabsBar;Corner(b,6)
 ...
 local Spy=Tab("Spy Monitor",1);local Loc=Tab("Location",2);local Settings=Tab("Settings",3);Tabs["Spy Monitor"].TextColor3=Config.TextPrimary;Tabs["Spy Monitor"].BackgroundColor3=State.accent;Spy.Visible=true
 ...
 local Code=Instance.new("TextLabel");Code.Size=UDim2.new(1,-10,0,0);Code.AutomaticSize=Enum.AutomaticSize.Y;Code.Position=UDim2.new(0,5,0,5);Code.BackgroundTransparency=1;Code.Text="-- Bir Event veya Remote seçin";Code.TextColor3=Config.TextPrimary;Code.Font=Enum.Font.Code;Code.TextSize=10;Code.TextXAlignment=Enum.TextXAlignment.Left;Code.TextYAlignment=Enum.TextYAlignment.Top;Code.Parent=View
 ...
 Clear.MouseButton1Click:Connect(function()for _,l in ipairs(State.logs)do pcall(l.button.Destroy,l.button)end;State.logs={};State.selected=nil;Code.Text="-- Bir Event veya Remote seçin"end)
 ...
 DisabledDecompile.MouseButton1Click:Connect(function()
    ...
    if not scr then Code.Text="-- Script bulunamadı" return end
    local src=nil
    if type(decompile)=="function" then pcall(function() src=decompile(scr) end) end
    if not src and type(getscriptbytecode)=="function" then pcall(function() local b=getscriptbytecode(scr);src="-- Bytecode: "..tostring(#b).." byte" end) end
    if src then Code.Text="-- "..scr:GetFullName().."\n\n"..src:sub(1,8000)
    else Code.Text="-- Decompiler bu executor'da yok" end
 end)
 ...
 -- Türkçe: Debug blacklist tablolarını temizler.
 ...
 -- Türkçe: Block listeleri Safe Debug içinde yalnızca gözlem amaçlı tutulur; ağ çağrısı değiştirilmez.
...
ToggleButton(Autoblock,"autoblock","Autoblock")
 ...
 DisableInfo.MouseButton1Click:Connect(function()
    ...
    Status.Text=State.showInfo and "● DEBUG • HOOK ACTIVE" or ""
end)
 ...
 -- Türkçe: Remote çağrı sıklığını güvenli şekilde ölçer; Safe Debug hiçbir çağrıyı engellemez.
 local function ObserveAutoblock(remote)
  ...
  pcall(function()
   ...
   if State.callCounts[name]>=10 then State.blockedNames[name]=true;State.callCounts[name]=0;blocked=true;Status.Text=State.showInfo and ("Auto-blocked: "..name) or "";task.delay(3,function()if Gui.Parent and State.showInfo then Status.Text="● DEBUG • HOOK ACTIVE" end end) end
  end)
  return blocked or (remote.Name and State.blockedNames[remote.Name]) or false
 end

 -- Türkçe: FireServer/InvokeServer çağrılarını izler ve block listesine göre engelleyebilir.
 local function InstallHook()
  local old
  if type(hookmetamethod)=="function" then
   local ok,r=pcall(function()return hookmetamethod(game,"__namecall",Closure(function(self,...)
    local m=Method()
    if Gui.Parent and (m=="FireServer" or m=="InvokeServer") then
        if State.blockedInstances[self] then return nil end
        if self.Name and State.blockedNames[self.Name] then return nil end
   ...
   if ok and r then Status.Text=State.showInfo and "● DEBUG • HOOK ACTIVE" or "";return true end
  end
  if type(getrawmetatable)=="function" then
   local ok=pcall(function()local mt=getrawmetatable(game);old=mt.__namecall;if type(setreadonly)=="function" then setreadonly(mt,false)elseif type(make_writeable)=="function" then make_writeable(mt)end;mt.__namecall=Closure(function(self,...)
    local m=Method()
    ...
    if self.Name and State.blockedNames[self.Name] then return nil end
    if ObserveAutoblock(self) then return nil end
    if State.excludedInstances[self] or (self.Name and State.excludedNames[self.Name]) then return old(self,...) end
    if Caller() and not State.logCaller then return old(self,...) end
    local args={...};AddLog(self,m,args,CaptureCaller());return old(self,...)
   end);if type(setreadonly)=="function" then setreadonly(mt,true)elseif type(make_readonly)=="function" then make_readonly(mt)end end)
   if ok then Status.Text=State.showInfo and "● DEBUG • HOOK ACTIVE" or "";return true end
  end
  Status.Text="● HOOK DESTEKLENMİYOR";Status.TextColor3=Config.CloseRed;task.delay(3,function()Status.TextColor3=Config.EventColor end);return false
 end
